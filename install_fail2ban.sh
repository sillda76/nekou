...
# 功能：
#  1. 安装 fail2ban
#  2. 查看 fail2ban 运行状态 (使用 sudo service fail2ban status)
#  3. 查看 SSH 服务封禁情况
#  4. 查看 fail2ban 配置文件
#  5. 实时查看 fail2ban 日志
#  6. 手动封禁/解封 IP 地址
...
#===== 显示 fail2ban 运行状态（原先的函数，仅在安装后显示详细状态）=====
...
function tail_log() {
    echo -e "${GREEN}实时查看 fail2ban 日志（按 q 键退出）:${RESET}"
