...
get_public_ip() {
    ...
    if [[ -n "$ipv4" ]]; then
        ...
        isp=$(curl -s --max-time 3 ipinfo.io/org)
        echo -e "${GREEN}运营商:${NC} $isp"
...
uninstall() {
    ...
    if grep -q '# GIT BRANCH AND CUSTOM PS1 START' ~/.bashrc; then
        echo -e "${YELLOW}清理 ~/.bashrc 自定义 PS1 配置...${NC}"
        sed -i '/# GIT BRANCH AND CUSTOM PS1 START/,/# GIT BRANCH AND CUSTOM PS1 END/d' ~/.bashrc
    fi
...
EOF
    ...
    if ! grep -q '# GIT BRANCH AND CUSTOM PS1 START' ~/.bashrc; then
        echo "" >> ~/.bashrc
        echo '# GIT BRANCH AND CUSTOM PS1 START' >> ~/.bashrc
        ...
        echo '# GIT BRANCH AND CUSTOM PS1 END' >> ~/.bashrc
        echo -e "${GREEN}自定义 PS1 已添加到 ~/.bashrc${NC}"
    fi
