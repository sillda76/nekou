...
EOF
    ...
    # 询问ASN显示选项
    while true; do
        echo -e "${ORANGE}请选择要显示的ASN信息：${NC}"
        echo "1. 显示IPv4的ASN和运营商"
        echo "2. 显示IPv6的ASN和运营商"
        echo "0. 取消安装并回滚"
    ...
    if [[ -n "$ipv4" ]]; then
        ...
        if [[ -n "$asn_info" ]]; then
            echo -e "${CYAN}ASN (IPv4): $asn_info${NC}"
        fi
    fi'
                break
                ;;
            2)
                asn_code='
    if [[ -n "$ipv6" ]]; then
        asn_info=$(curl -s --max-time 3 -H "Authorization: Bearer 3b01046f048430" "https://ipinfo.io/$ipv6/org")
        if [[ -n "$asn_info" ]]; then
            echo -e "${CYAN}ASN (IPv6): $asn_info${NC}"
        fi
    fi'
                break
                ;;
            0)
                echo -e "${YELLOW}取消安装并回滚...${NC}"
                uninstall
                exit 0
                ;;
            *)
    ...
    # 替换ASN代码
    sed -i "s|%ASN_CODE%|$asn_code|g" ~/.local/sysinfo.sh
