...
# Placeholder for get_public_ip call
EOF
    ...
    echo -e "${YELLOW}安装完成！请选择显示的 ASN 和运营商信息：${NC}"
    echo -e "${ORANGE}1. 显示 IPv4 ASN 和运营商${NC}"
    echo -e "${ORANGE}2. 显示 IPv6 ASN 和运营商${NC}"
    echo -e "${ORANGE}0. 取消安装${NC}"
    read -p "请输入选项 (0、1 或 2): " choice

    case $choice in
        1)
            echo 'get_public_ip "ipv4"' >> ~/.local/sysinfo.sh
            ;;
        2)
            echo 'get_public_ip "ipv6"' >> ~/.local/sysinfo.sh
            ;;
        0)
            echo -e "${YELLOW}正在取消安装...${NC}"
            uninstall
            return
            ;;
        *)
            echo -e "${RED}错误：无效选项，取消安装。${NC}"
            uninstall
            return
            ;;
    esac
